...
<Window {options} bind:this={window} on_click_close={destroy}>
    <div
        bind:this={content_node}
        on:mousemove={on_user_active}
        slot="content"
        class="absolute inset-0 flex flex-col bg-xp-yellow overflow-hidden"
    >
        <p
            class="z-10 absolute top-2 left-1/2 -translate-x-1/2 p-2 text-center hidden rounded bg-black/70 text-white font-sans"
        ...
        {:else if player_node != null && file_type == 'audio'}
            <div class="grow bg-black w-full">
                <iframe
                    ...
                    src="/html/visualizers/{utils.random_int(1, 12)}.html"
                ></iframe>
            </div>
        {/if}
        ...
        <div
            class="{fullscreen
                ? 'absolute bottom-0 left-0 right-0 h-[102px]'
                : 'shrink-0 w-full'} 
        ...
        >
            ...
            <div
                class="shrink-0 w-full flex flex-row justify-between items-center"
            >
                <div class="flex flex-row items-center">
                    ...
                    <button
                        ...
                        disabled={player_node == null || isNaN(duration)}
                        class="w-5 h-5 p-1 m-1 group outline-none"
                        on:click={play}
                    >
                        {#if player_node == null || paused == null || paused}
                            <!-- play icon -->
                            <svg
                                class="w-full h-full fill-slate-900 group-disabled:fill-slate-400"
                    ...
                    >
                        <svg
                            class="w-full h-full fill-slate-900 {loop
                                ? ''
                                : 'fill-slate-400'} group-disabled:fill-slate-400"
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 512 512"
            ...
            <div
                class="shrink-0 bg-black flex flex-row justify-between items-center p-2 basis-8"
            >
                {#if player_node != null}
                    {#if isNaN(duration)}
                        <div class="text-[12px] text-slate-50">
                            Buffering...
                        </div>
                    {:else}
                        <div class="text-[12px] text-slate-50">
                            {paused == null || paused ? 'Paused' : 'Playing'}
                        </div>
                        <div class="text-[12px] text-slate-50">
                            {utils.format_time(currentTime)}/{utils.format_time(
                                duration,
                            )}
                        </div>
                    {/if}
