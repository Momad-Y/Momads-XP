...
<div
    ...
    style:background={program.window?.z_index == $zIndex
        ? 'rgb(30,82,183)'
        : 'rgb(60,129,243)'}
    style:box-shadow={program.window?.z_index == $zIndex
...
>
    <img
        src={program.options.icon}
        width="15px"
        height="15px"
        class="shrink-0 ml-2 pointer-events-none"
        alt=""
    />
    <p
        class="text-[11px] text-slate-50 text-ellipsis mx-1 line-clamp-1 leading-none"
    >
