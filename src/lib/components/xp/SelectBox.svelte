        ...
        if (direction == 'bottom') {
            ...
            if (
                document.body.offsetHeight -
                    node_ref.getBoundingClientRect().y >
                150
            ) {
                dropbox_pos = 'top:100%;';
            } else {
...
<div
    bind:this={node_ref}
    class="bg-slate-50 h-6 text-slate-800 border border-blue-300 p-1 text-[11px] absolute"
    {style}
    use:click_outside
    on:click_outside={() => (expand = false)}
>
    <div
        class="absolute bg-slate-50 w-full left-0 max-h-[100px] overflow-y-auto
        box-content border border-slate-300 {expand ? 'block' : 'hidden'}"
        style:height="{expand
            ? Math.min(items.length * item_height, 100)
            : 0}px"
        style={dropbox_pos}
    >
    ...
    <div
        class="absolute inset-0 flex flex-row items-center pl-2"
    ...
    {#if items.length > 1}
        <div
            class="absolute right-[1px] top-[1px] bottom-[1px] p-1 rounded bg-blue-200 border-2 border-slate-50 box-border"
            style:width="{item_height - 2}px"
            on:click={on_click_expand}
        >
            <svg
                class="w-full h-full fill-blue-700"
