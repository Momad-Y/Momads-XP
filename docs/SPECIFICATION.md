...
- Black background with the **custom Momad's-XP logo image** (`static/assets/images/xp-logo.png`; owner decision 2026-07-18: the custom logo image everywhere the Windows logo would appear — but no extra HTML branding text lines on any screen)
...
- [ ] **Decide whether `static/html/jspaint/src/sessions.js` should ship at all** (owner-requested 2026-08-31, after GitHub secret-scanning alert #1). The alert flagged a Google API key in that file at `8ae7c39` — jspaint's OWN Firebase config (`projectId: firebase-jspaint`), vendored in with the MIT bundle and already public upstream, so it was never ours to rotate or revoke. It was removed from shipped code in `e47fc2f` (#115) and the alert now points only at history. **This item is therefore hygiene, not remediation** — deleting the file removes the last place a vendored third-party credential could hide.

      The obvious framing ("it only exists for `window.new_local_session`, so inline that and delete it") is WRONG, and checking that is step one. The file is 270 lines and carries a live `LocalSession` class doing localStorage autosave, a data-loss recovery window, and URL-hash session routing. `new_local_session` (`:258`) does not create a session — it calls `change_url_param("local", …)` and lets the hash handler construct one. So the real choice is:
      1. **Keep it.** Zero risk, and autosave keeps working. The key is already gone, so the security argument is spent.
      2. **Replace it with a stub** that satisfies the two unguarded call sites — `open_from_image_info` (the path `paint.svelte` uses to open a VFS image) and `file_new`, at `functions.js:798` and `:916` in the SHIPPED tree — accepting the loss of jspaint's localStorage autosave and recovery. Paint's canvas already round-trips through OUR VFS, so autosave may be redundant; confirm before assuming.
      3. **Port autosave** onto the VFS, which is a feature, not a prune.

      Whatever is chosen, `e2e/paint.spec.ts` is the guard: `scripts/prune-jspaint.mjs:18-25` records that deleting the file leaves the reopened canvas blank and that the spec goes red on it, mutation-verified at Phase 3 gate 4. Do not touch this without running it. (That comment cites `:805`/`:923` — the PRE-prune line numbers, since steps E2/E3 of the same script delete blocks from `functions.js`. Both are correct for their own tree; work from the shipped numbers above.)
