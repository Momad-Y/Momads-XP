...
- [x] **`static/html/jspaint/src/sessions.js` replaced with a no-op stub** (owner-requested 2026-08-31; done the same day rather than deferred). Raised by GitHub secret-scanning alert #1, which flagged a Google API key in that file at `8ae7c39` — jspaint's OWN Firebase config (`projectId: firebase-jspaint`), vendored with the MIT bundle and already public upstream, so it was never ours to rotate. That key had already left shipped code in `e47fc2f` (#115); **alert #1 is closed as won't-fix**, since it pointed only at history and a rewrite would break every clone and fork without un-publishing anything.

      The real find was underneath it. jspaint's `LocalSession` autosave **never worked in this app and was leaking**: `paint.svelte` loads jspaint with no session in the hash, so `sessions.js` minted a fresh random id on every open (`generate_session_id` is `Math.random()`) and wrote `image#<random>` to localStorage without ever reading the previous one back. Two consecutive Paint windows produced two unrelated keys — verified in a browser, not assumed. It was write-only: one orphaned PNG data URL per Paint window, accumulating against the ~5 MB quota, with jspaint's own `storage_quota_exceeded` dialog waiting at the end.
