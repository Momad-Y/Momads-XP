...
- **Gate-6 red-team disposition** (verdict: merge with fixes — all applied or tracked):
    - Fixed in this PR: seed re-seed now falls back to the cached drive on fetch failure (offline boot-brick regression); dead "Tour XP" tray icon removed; `tooltip.ts` uses `textContent` (latent XSS sink); `app.html` CDN scripts pinned + SRI hashes added (loadjs was `@latest`)
    - **Tracked for Phase 1 (must-do):** Font Awesome **Pro** SVG paths are inlined in several inherited components (SMenu, SelectBox, ContextMenu, RButton, image_viewer, …) — a commercial-license violation for a public site; replace with FA Free equivalents during Phase 1's parity work (blind swaps risk visual regressions, so it belongs with the parity loop)
    - Tracked for Phase 1: SRI/vendoring for the `loadjs`-loaded scripts in `desktop.svelte` (gstatic charts, panzoom) — **DONE**: panzoom was vendored in Phase 3 (design decision 13) and the gstatic charts loader was removed entirely post-Phase 3, see `docs/cdn-removal-plan.md`; the unhandled-extension fallback still points at pruned `notepad.svelte` (currently unreachable — every seeded item has a handler — but wire it to a dialog or kept viewer when touched)
    - TS-conversion exceptions (documented honesty gap): `src/routes/xp/programs/my_computer/sort.js` (web worker) and `src/hooks.server.js` remain JS, outside the typed lint zone
