...
- **Gate-6 red-team disposition** (verdict: merge with fixes — all applied or tracked):
    ...
    - Tracked for Phase 1: SRI/vendoring for the `loadjs`-loaded scripts in `desktop.svelte` (gstatic charts, panzoom); the unhandled-extension fallback still points at pruned `notepad.svelte` (currently unreachable — every seeded item has a handler — but wire it to a dialog or kept viewer when touched)
